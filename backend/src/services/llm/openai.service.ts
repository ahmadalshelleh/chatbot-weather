...
export class OpenAIService {
  ...
  async *chatStream(messages: Message[], tools: Tool[]): AsyncGenerator<string, LLMResponse, unknown> {
    try {
      const systemMessage = {
        role: 'system',
        content: `You are GPT-3.5 Turbo by OpenAI, a friendly and professional weather assistant with access to real-time weather data.
...
When asked which AI you are, identify yourself as GPT-3.5 Turbo (OpenAI).`
      };
